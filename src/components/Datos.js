...
async function infoOpenAI(locationOrPostalCode, weatherData) {
    ...
    const userMessage = `Analiza los siguientes datos del tiempo para hoy, la tendencia de las temperaturas y % de lluvias para los próximos dias y resume el tiempo en ${locationOrPostalCode} hoy y la tendencia prevista. Por debajo de un 5% NO se considera posibilidad real de lluvia. Probabilidad lluvia: 
    ${dailyData.precipitation_probability_mean[0]}%, Temp.: ${temperature}°C, Viento: ${windspeed}km/h. Pronóstico para proximos días: Máx.: ${maxTemperatures.join('°C,')}°C, Mín.: ${minTemperatures.join('°C,')}°C,  Lluvia: ${rainProbabilities.join('%,')}%, 
    con un TAMAÑO MAXIMO de respuesta de 250 caracteres y SIN DAR CIFRAS. Un ejemplo de posible estructura:¡Hola! Hoy hace un dia fresco en (nombrar ciudad pero NO el codigo postal) con cielos despejados, con temperaturas en aumento y algo de brisa. 
    Para mañana y los dias venideros se espera que haga más frío y seguramente vuelvan las lluvias. Posibilidad de tormentas !Abrigate!`;
